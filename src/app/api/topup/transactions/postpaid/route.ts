...
export async function POST(req: Request) {
  ...
  try {
    ...
    const { inquiry_id, idempotency_key } = body;

    if (!inquiry_id) {
      return NextResponse.json({ success: false, error: "Inquiry ID wajib disertakan" }, { status: 400 });
    }

    const idemKey = idempotency_key || `idem-${Date.now()}-${Math.random().toString(36).slice(2, 9)}`;

    const result = await topupOrchestrator.processBillPayment({
    ...
    console.error("Postpaid Transaction API Error:", error);
    return NextResponse.json({ success: false, error: error.message || "Gagal memproses pembayaran tagihan" }, { status: 400 });
  }
}
